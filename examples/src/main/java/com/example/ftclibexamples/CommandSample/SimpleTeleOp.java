...
/**
 * Does the same thing as {@link SampleTeleOp} but is much simpler in scope.
 *
 * <p>Note that the <b>proper</b> way to do this is with the SampleTeleOp version, where most things
 * are set up as commands/subsystems to avoid potential drawbacks of the {@link InstantCommand}.
 */
@TeleOp
@Disabled
public class SimpleTeleOp extends CommandOpMode {
  ...
  private DriveSubsystem drive = new DriveSubsystem(hardwareMap, "left", "right", 100.0);
  private DefaultDrive driveCommand =
      new DefaultDrive(drive, driverOp::getLeftY, driverOp::getRightX);
