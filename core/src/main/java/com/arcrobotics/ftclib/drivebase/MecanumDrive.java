 ...
 * The drive methods {@link #driveRobotCentric(double, double, double)} and {@link
 * #driveFieldCentric(double, double, double, double)} are meant to be put inside of a loop. You can
 * call them in {@code void loop()} in an OpMode and within a {@code while (!isStopRequested() &&
 * opModeIsActive())} loop in the {@code runOpMode()} method in LinearOpMode.
 *
 * <p>For the derivation of mecanum kinematics, please watch this video:
...
public class MecanumDrive extends RobotDrive {
  ...
  /**
   * Sets up the constructor for the mecanum drive. Automatically inverts right side by default
   *
   ...
   */
  public MecanumDrive(
      boolean autoInvert, Motor frontLeft, Motor frontRight, Motor backLeft, Motor backRight) {
  ...
  public void driveRobotCentric(
      double strafeSpeed, double forwardSpeed, double turnSpeed, boolean squareInputs) {
  ...
  /**
   * Drives the robot from the perspective of the driver. No matter the orientation of the robot,
   * pushing forward on the drive stick will always drive the robot away from the driver.
   *
   * @param strafeSpeed the horizontal speed of the robot, derived from input
   * @param forwardSpeed the vertical speed of the robot, derived from input
   * @param turnSpeed the turn speed of the robot, derived from input
  ...
  public void driveFieldCentric(
    ...
    motors[MotorType.kFrontRight.value].set(
        wheelSpeeds[MotorType.kFrontRight.value] * rightSideMultiplier * maxOutput);
    motors[MotorType.kBackLeft.value].set(wheelSpeeds[MotorType.kBackLeft.value] * maxOutput);
    motors[MotorType.kBackRight.value].set(
        wheelSpeeds[MotorType.kBackRight.value] * rightSideMultiplier * maxOutput);
  }
   ...
   */
  public void driveFieldCentric(
      double xSpeed, double ySpeed, double turnSpeed, double gyroAngle, boolean squareInputs) {
