...
/**
 * A differential drive is one that has two motors or motor groups on either side of the robot. Each
 * side acts as a connected set. You can see {@link MotorGroup} for more details as to how that
 * might work. There are two types of drive systems here. You can use tank and arcade.
 *
 * <p>Arcade drive use a y-value input from the controller and a value from the turn stick. We know
 * that when the turn stick is pushed left, the right side should move forward and the left side
 * should move backwards. Therefore, since pushing the turn stick to the left returns a negative
 * value, it should be added to the left speed and subtracted from the right speed. <br>
 * <br>
 * Tank drive uses a y-value input from the left and right sticks. The sticks control their
 * respective side of the robot.
 */
public class DifferentialDrive extends RobotDrive {
   ...
   *
   * <p>To pass multiple motors per side, use a {@link MotorGroup}. If a motor needs to be inverted,
   * do so before passing it in. By default, the right motor speed multiplier is {@value
   * #kDefaultRightSideMultiplier}.
   *
   * @param myMotors The {@link Motor} objects in the differential drive. Must be in the order of
   *     left, right.
   */
