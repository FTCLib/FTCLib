...
/**
 * This is an optional feature of paths. A triggered action is an action that called when some
 * condition is met. For example, you might have a triggered action that triggers when the robot
 * move above some Y value on the field.
 *
 * @see Path
 * @version 1.1
...
public abstract class TriggeredAction {
  ...
  /**
   * Called regularly by the path it is apart of. If the trigger condition is met this will call
   * doAction().
   */
  public void loop() {
    if (isTriggered()) {
   ...
   *
   * @return true if the trigger condition is met and the action should be performed, false
   *     otherwise.
   */
