 ...
 * calculation required to traverse the path. A path can be implemented in two different way. The
 * first way is running the loop/updating motor powers manually. This is a good implementation for
 * those teams who wish to just use the pure pursuit section FtcLib and not much else. The other
 * implementation is using the automatic feature. This features does all the hard work for you.
 * Teams who use FtcLib's features to power most of their code will find this method appealing.
 *
 * <p>To learn how to implement a pure pursuit path, please see the example implementations and
 * tutorials.
 *
 * @see Waypoint
 * @version 1.3
...
public class Path extends ArrayList<Waypoint> {
  ...
  public Path(Waypoint... waypoints) {
    for (Waypoint waypoint : waypoints) add(waypoint);
   ...
   *
   * <p>In order for a path to be considered legal it must have: - At least 2 waypoints. - Begin
   * with a StartWaypoint - End with an EndWaypoint - Not contain any StartWaypoints or EndWaypoints
   * in it's body.
   *
  ...
  public void init() {
    ...
    for (int i = 1; i < size(); i++) ((GeneralWaypoint) get(i)).inherit(get(i - 1));
    // Mark the init as complete.
    initComplete = true;
  }

  /**
   * Initiates the automatic path following feature. The robot will follow the path and perform
   ...
   * @return True if the path completed successfully, false if the path did not (timed out, lost
   *     path, etc.).
   * @throws IllegalStateException If automatic mode is disabled/not configured or the init has not
   *     been ran.
   */
  public boolean followPath(MecanumDrive mecanumDrive, Odometry odometry) {
    ...
    while (!isFinished()) {
      ...
      double[] motorPowers =
          loop(robotPosition.getX(), robotPosition.getY(), robotPosition.getHeading());
      // Update motor speeds.
      ...
      if (!isFinished()) {
        ...
        for (double power : motorPowers) if (power != 0) pathAborted = false;
  ...
  /**
   * This is the principle path method. After everything is configured and initiated, this method
   * can be used. Using the robot's horizontal, y, and rotation, this method calculates the
   * appropriate motor powers for the robot to follow the path. This method calls all
   * triggered/interrupted actions automatically. If this returns zero motor speeds {0, 0, 0} that
   * means the path has either (1) timed out, (2) lost the path and retrace was disabled, or (3)
   ...
   * @return A double array containing the motor powers. a[0] is the x power, a[1] is the y power,
   *     and a[2] is the turn power.
   */
  public double[] loop(double vPosition, double hPosition, double rotation) {
    // First, make sure the init has been called. While this does not guarantee the program will run
    // without errors, it is better than nothing.
    if (!initComplete)
    ...
    for (int i = 1; i < size(); i++) {
      ...
      List<Translation2d> points =
          PurePursuitUtil.lineCircleIntersection(robotPosition, radius, linePoint1, linePoint2);
      for (Translation2d point : points)
        // Add results to list.
        intersections.add(new TaggedIntersection(point, get(i), i));
      if (get(i) instanceof PointTurnWaypoint) {
        // If the second waypoint is a point turn waypoint, decrease the follow radius so the next
        // point is always found.
        ...
        if (adjustedRadius < radius) {
          ...
          intersections.add(
              new TaggedIntersection(((PointTurnWaypoint) get(i)).getTranslation(), get(i), i));
        }
      }
    ...
    switch (pathType) {
      case HEADING_CONTROLLED:
        bestIntersection =
            selectHeadingControlledIntersection(
                intersections, new Pose2d(vPosition, hPosition, new Rotation2d(rotation)));
        break;
    ...
    if (bestIntersection.taggedPoint.getTimeout() != -1)
      ...
      if (System.currentTimeMillis()
          > lastWaypointTimeStamp + bestIntersection.taggedPoint.getTimeout()) {
        timedOut = true;
        // If it has, return no motor speeds.
        return new double[] {0, 0, 0};
      }
    // After the best intersection is found, the robot behaves differently depending on the type of
    // waypoint.
  ...
  private double[] retrace(double xPosition, double yPosition, double rotation) {
    ...
    double[] motorPowers =
        PurePursuitUtil.moveToPosition(
            xPosition,
            yPosition,
            rotation,
            lastKnownIntersection.getX(),
            lastKnownIntersection.getY(),
            rotation,
            false);
  ...
  /**
   * Selects and returns the "best" intersection from the given list using heading control. The
   * intersection is chosen based on the following rules: 1. If the list contains any untraversed
   * waypoints, they are given priority and the best intersection is the point closest to the point
   * turn waypoint. 2. If the list contains no point turn points, then it chooses the intersection
   * the robot is oriented most closely towards.
   *
   ...
   */
  private TaggedIntersection selectHeadingControlledIntersection(
      List<TaggedIntersection> intersections, Pose2d robotPos) {
    TaggedIntersection bestIntersection = intersections.get(0);
    boolean pointTurnPriority = false;
    /**
     * In a heading controlled path, the intersection the robot is most closely oriented toward is
     * considered the "best point".
     */
    for (TaggedIntersection intersection : intersections) {
      ...
      if (intersection.taggedPoint instanceof PointTurnWaypoint) {
        ...
        if (!ptwaypoint.hasTraversed()) {
          // If point turn waypoint is found, and it has not already been traversed, then it takes
          // priority.
          pointTurnPriority = true;
          if (!(bestIntersection.taggedPoint instanceof PointTurnWaypoint))
            bestIntersection = intersection;
          else {
            // If two intersections associated with a point turn waypoint are found, choose the one
            ...
            if (bestIntersection.waypointIndex < intersection.waypointIndex)
              ...
              if (PurePursuitUtil.isInFront(
                  get(intersection.waypointIndex - 1).getPose().getTranslation(),
        ...
        double absoluteAngleToIntersection =
            Math.atan2(intersection.intersection.getY(), intersection.intersection.getX());
        double relativeAngleToIntersection = absoluteAngleToIntersection - robotPos.getHeading();
        // Relative angle to best intersection.
        double absoluteAngleToBestIntersection =
            Math.atan2(bestIntersection.intersection.getY(), bestIntersection.intersection.getX());
  ...
  /**
   * Selects and returns the "best" intersection from the given list by choosing the intersection
   * that is farthest along the path. The intersection is chosen based on the following rules: 1. If
   * the list contains any untraversed waypoints, they are given priority and the best intersection
   * is the point closest to the point turn waypoint. 2. If the list contains no point turn points,
   * then it chooses the intersection that is farthest along the path.
   *
   * @param intersections Intersection list.
   * @return The best intersection in the form of a TaggedIntersection.
   */
  private TaggedIntersection selectWaypointOrderingControlledIntersection(
      List<TaggedIntersection> intersections) {
    TaggedIntersection bestIntersection = intersections.get(0);
    boolean pointTurnPriority = false;
    /**
     * In a waypoint ordering controlled path, the intersection that is farthest along the path is
     * considered the "best point".
     */
    for (TaggedIntersection intersection : intersections) {
      // Check to see if a point turn waypoint is found.
  ...
  private double[] handleGeneralIntersection(TaggedIntersection intersection, Pose2d robotPos) {
    /**
     * General intersections are handled like normal pure pursuit intersections. The robot simply
     * moves towards them.
     */
  ...
  /**
   * Returns the motor speeds required to approach the given point turn intersection. This will
   * cause the robot to behave as follows: 1. Approach and decelerate to the waypoint. 2. Perform a
   * point turn. 3. Continue to the next waypoint as normal.
   *
   * @param intersection Intersection to approach.
   * @param robotPos Robot's current position/rotation.
   * @return Final motor speeds.
  ...
  private double[] handlePointTurnIntersection(TaggedIntersection intersection, Pose2d robotPos) {
    /**
     * Point turn intersections are handled very differently than general intersections. Instead of
     * "curving" around the point, the robot will decelerate and perform a point turn.
     */
    ...
    double ta;
    double[] motorPowers;
    if (!waypoint.hasTraversed()
        && PurePursuitUtil.positionEqualsWithBuffer(
            robotPos.getTranslation(), waypoint.getTranslation(), waypoint.getPositionBuffer())) {
      // If the robot has not reached the point.
      if (((GeneralWaypoint) get(intersection.waypointIndex + 1)).usingPreferredAngle()) {
        if (PurePursuitUtil.rotationEqualsWithBuffer(
            robotPos.getHeading(),
            ((GeneralWaypoint) get(intersection.waypointIndex + 1)).getPreferredAngle(),
            waypoint.getRotationBuffer()))
          // If the robot has reached the point and is at the preferredAngle, then the point is
          // traversed.
        ...
        double tempTy =
            ((GeneralWaypoint) get(intersection.waypointIndex + 1))
                .getPose()
                .getTranslation()
                .getY();
        double tempTx =
        ...
        if (PurePursuitUtil.rotationEqualsWithBuffer(
            robotPos.getHeading(), ta, waypoint.getRotationBuffer()))
          // If the robot has reached the point and is at the target angle, then the point is
          // traversed.
          waypoint.setTraversed();
      }
  ...
  /**
   * Returns the motor speeds required to approach the given interrupt intersection. This will cause
   * the robot to behave as follows: 1. Approach and decelerate to the waypoint. 2. Perform a point
   * turn / align with the preferred angle. 4. Perform the interrupt action. 3. Continue to the next
   * waypoint as normal.
   *
   * @param intersection Intersection to approach.
   * @param robotPos Robot's current position/rotation.
  ...
  private double[] handleInterruptIntersection(TaggedIntersection intersection, Pose2d robotPos) {
    /**
     * Interrupt intersections are handled similarly to point turn intersections. Instead of
     * continuing directly after it has turned, the robot will stop and perform the interrupt
     * actions.
     */
    ...
    if (!waypoint.hasTraversed()
      ...
      if (waypoint.getType() == WaypointType.END) {
        if (waypoint.usingPreferredAngle()
            && !PurePursuitUtil.rotationEqualsWithBuffer(
                robotPos.getHeading(), waypoint.getPreferredAngle(), waypoint.getRotationBuffer()))
          ta = waypoint.getPreferredAngle();
        else {
  ...
  /**
   * Returns the motor speeds required to approach the given end intersection. This will cause the
   * robot to behave as follows: 1. Approach and decelerate to the end point. 2. Turn to face the
   * preferred angle (if provided). 3. Mark the path as complete.
   *
   * @param intersection Intersection to approach.
   * @param robotPos Robot's current position/rotation.
   * @return Final motor speeds.
  ...
  /**
   * Sets the path type to the specified type. By default the path type is
   * WAYPOINT_ORDERING_CONTROLLED. This is not recommended, do not change the path type unless you
  ...
  public Path setMotionProfile(PathMotionProfile profile) {
    if (profile == null) throw new NullPointerException("The motion profile connot be null");
    motionProfile = profile;
    return this;
  }

  /**
   * Sets the timeouts of n waypoints where n is the amount of arguments provided. The nth waypoint
   * timeout is set the the nth argument given.
   *
  ...
  public Path setWaypointTimeouts(long... timeouts) {
    for (int i = 0; i < size() && i < timeouts.length; i++)
      if (get(i) instanceof GeneralWaypoint) ((GeneralWaypoint) get(i)).setTimeout(timeouts[i]);
    return this;
  }

  /**
   * Sets the timeout for each individual waypoint to be the value provided. This is not
   * recommended.
   *
  ...
  public Path setWaypointTimeouts(long timeout) {
    for (Waypoint waypoint : this)
      if (waypoint instanceof GeneralWaypoint) ((GeneralWaypoint) waypoint).setTimeout(timeout);
    return this;
  }

  /**
   * Configures the retrace settings. The default values are as follows: movementSpeed = 1 turnSpeed
   * = 1
   *
  ...
  /**
   * Enables retrace. If the robot loses the path and this is enabled, the robot will retrace its
   * moves to try to re find the path. This is enabled by default.
   *
   * @return This path, used for chaining methods.
   */
  ...
  public Path addTriggeredActions(TriggeredAction... actions) {
    for (TriggeredAction triggeredAction : actions) triggeredActions.add(triggeredAction);
    return this;
  }
  ...
  public void reset() {
    resetTimeouts();
    for (Waypoint waypoint : this)
      if (waypoint instanceof GeneralWaypoint) ((GeneralWaypoint) waypoint).reset();
    for (TriggeredAction actions : triggeredActions) actions.reset();
  }
  ...
  private void verifyLegality() {
    if (size() < 2) throw new IllegalStateException("A path must have at least two waypoints.");
  ...
  private void loopTriggeredActions() {
    for (TriggeredAction action : triggeredActions) action.loop();
  }

  /** Performs all queued interrupt actions. */
  private void runQueuedInterruptActions() {
    while (!interruptActionQueue.isEmpty()) interruptActionQueue.remove().performAction();
  }
   ...
   */
  private void adjustSpeedsWithProfile(
      double[] speeds, TaggedIntersection intersection, Translation2d robotPos) {
  ...
  private PathMotionProfile getDefaultMotionProfile() {
    if (defaultMotionProfile != null) return defaultMotionProfile;
    else
      ...
      return new PathMotionProfile() {
        ...
        public void decelerate(
            ...
            double speed,
            double configuredMovementSpeed,
            double configuredTurnSpeed) {
  ...
  private void normalizeMotorSpeeds(double[] speeds) {
    ...
    if (speeds[2] > 1) speeds[2] = 1;
    else if (speeds[2] < -1) speeds[2] = -1;
  }
