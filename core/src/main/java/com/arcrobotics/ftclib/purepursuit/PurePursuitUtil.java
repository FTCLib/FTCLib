...
public final class PurePursuitUtil {
  ...
  public static double angleWrap(double angle) {
    if (angle > 0) return ((angle + Math.PI) % (Math.PI * 2)) - Math.PI;
    else return ((angle - Math.PI) % (Math.PI * 2)) + Math.PI;
  }

  /**
   * Calculates if a point is further along a line then another point. Useful for determining the
   * best intersection point in pure pursuit. Both points are assumed to lie on the line.
   *
   ...
   */
  public static boolean positionEqualsWithBuffer(
      Translation2d p1, Translation2d p2, double buffer) {
    if (p1.getX() - buffer < p2.getX() && p1.getX() + buffer > p2.getX())
      if (p1.getY() - buffer < p2.getY() && p1.getY() + buffer > p2.getY()) return true;
    return false;
  }
  ...
  /**
   * Takes the robot's current position and rotation and calculates the motor powers for the robot
   ...
   * @return A double array containing raw motor powers. a[0] is strafe power, a[1] is vertical
   *     power and a[2] is turn power.
   */
  public static double[] moveToPosition(
      double cx, double cy, double ca, double tx, double ty, double ta, boolean turnOnly) {
    ...
    double powerX =
        relativeXToPosition / (Math.abs(relativeXToPosition) + Math.abs(relativeYToPosition));
    double powerY =
        relativeYToPosition / (Math.abs(relativeXToPosition) + Math.abs(relativeYToPosition));
   ...
   */
  public static List<Translation2d> lineCircleIntersection(
    ...
    Translation2d p1 =
        new Translation2d(
            linePoint1.getX() - baX * abScalingFactor1, linePoint1.getY() - baY * abScalingFactor1);
    if (disc == 0) {
    ...
    if (allPoints == null) {
      Translation2d p2 =
          new Translation2d(
              linePoint1.getX() - baX * abScalingFactor2,
    ...
    for (Translation2d point : allPoints) {
      ...
      if (point.getX() <= maxX && point.getX() >= minX)
        if (point.getY() <= maxY && point.getY() >= minY) boundedPoints.add(point);
    }
