...
public class SimpleServo implements ServoEx {
  ...
  public void setPosition(double position) {
    if (position > maxPosition) servo.setPosition(maxPosition);
    else if (position < minAngle) servo.setPosition(minPosition);
  ...
  public void setInverted(boolean isInverted) {
    if (isInverted) servo.setDirection(Servo.Direction.REVERSE);
    else servo.setDirection(Servo.Direction.FORWARD);
  }

  @Override
  public boolean getInverted() {
    if (Servo.Direction.REVERSE == servo.getDirection()) return true;
    else return false;
  }
