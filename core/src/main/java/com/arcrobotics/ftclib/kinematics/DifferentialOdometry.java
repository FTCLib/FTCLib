...
public class DifferentialOdometry extends Odometry {
  ...
  public DifferentialOdometry(
      DoubleSupplier leftEncoder, DoubleSupplier rightEncoder, double trackWidth) {
    this(trackWidth);
    m_left = leftEncoder;
  ...
  /**
   * The constructor that specifies the track width of the robot but defaults the position to (0, 0,
   * 0).
   *
  ...
  public void updatePose() {
    updatePosition(m_left.getAsDouble(), m_right.getAsDouble());
  }
  ...
  public void updatePosition(double leftEncoderPos, double rightEncoderPos) {
    ...
    Rotation2d angle =
        previousAngle.plus(new Rotation2d((deltaLeftDistance - deltaRightDistance) / trackWidth));

    Pose2d newPose = robotPose.exp(new Twist2d(dx, 0.0, angle.minus(previousAngle).getRadians()));
