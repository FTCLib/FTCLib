...
/**
 * This is an example of vision using EasyOpenCv.This example will show how to convert color
 * spaces,draw rectangles, and extract the value inside the rectangle. For full docs:
 * https://opencv-java-tutorials.readthedocs.io/ For the library:
...
/**
 * This is the pipeline for our vision system,a pipeline is a distinct steps to process and analyze
 * the input(camera frames)
 */
public class ExamplePipeLine extends OpenCvPipeline {
  ...
  int[] left_rect = {1, 2, 3, 4};

  int[] right_rect = {1, 2, 3, 4};
  ...
  public Mat drawRectangle(Mat frame, int[] points, Scalar color, int thickness) {
    ...
    Imgproc.rectangle(
        frame, new Point(points[0], points[1]), new Point(points[2], points[3]), color, thickness);
  ...
  public Mat processFrame(Mat input) {
    /**
     * input which is in RGB is the frame the camera gives We convert the input frame to the color
     * space matYCrCb Then we store this converted color space in the mat matYCrCb For all the color
     * spaces go to https://docs.opencv.org/3.4/d8/d01/group__imgproc__color__conversions.html
     */
    ...
    /**
     * This will extract the value of the CB channel in both rectangles 0 is the Y channel, 1 is the
     * Cr, 2 is Cb
     */
    ...
    /**
     * We now average value and extract it so now left is the Cb value of the left rectangle and
